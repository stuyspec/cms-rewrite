...
function Create_Draft() {
  ...
  const new_draft_handler = async () => {
    ...
    const volume: string = (document.getElementById("new_volume") as any).value;
    ...
    const text: string = draftToHtml(convertToRaw(editorState.getCurrentContent()))
  ...
  const upload_cover_image = async () => {
    ...
    if (uploaded_files.length > 0) {
      ...
      const public_url = (await upload_image_helper(uploaded_file)) as string;
