...
router.post("/publish_article", isAdminMiddleware, async (req, res, next) => {
  try {
    ...
    if (!draft_data) {
      res.status(404);
      throw new Error(`A draft with an id of ${draft_id} does not exist.`);
    }
...
async function getArticleHandler(req, res, next) {
  try {
    ...
    const article_extras = await ArticleExtra.find({
      article: article._id,
    }).populate({ path: "contributors", select: "-password" });
...
router.post("/update_article", isAdminMiddleware, async (req, res, next) => {
  try {
    ...
    const new_extras = req.body.article_extras.map((v) => {
      return { ...v, _id: new mongoose.Types.ObjectId() };
    });
...
const editMiddleware = async function (req, res, next) {
  try {
    ...
    throw new Error("User must be an admin or the user must own this draft.");
  } catch (error) {
    next(error);
  }
};
...
async function get_staff_handler(req, res, next) {
  try {
    if (req.body.fuzzy_name) {
      ...
      res.json({
        staff: staff,
        description: "Successfully retrieved staff by doing a fuzzy search.",
      });
    } else {
